...
> 末尾の `bash thesis` の `bash` は `/bin/bash -c "..."` に渡す内部スクリプト名（`$0`）にあたるダミー引数で、続く `thesis` / `wr` / `latex` / `ise` / `poster` が文書タイプ（`$1`）として `setup.sh` に渡されます。
...
INDIVIDUAL_MODE=true /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/smkwlab/student-repo-management/main/create-repo/setup.sh)" bash latex
```
