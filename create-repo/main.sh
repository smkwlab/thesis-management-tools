...
# 管理リポジトリへのPR作成（ブランチ保護設定依頼）
create_protection_request_pr() {
    local student_id="$1"
    local repo_name="$2"
    local timestamp=$(date +%Y%m%d-%H%M%S)
    local branch_name="add-student-${student_id}-${timestamp}"
    
    echo "📋 ブランチ保護設定依頼PRを作成中..."
    
    # 管理リポジトリをクローン（一時ディレクトリ）
    ...
    if gh repo clone smkwlab/thesis-management-tools . -- --depth=1 --single-branch; then
        ...
        # 学生情報を追加
        echo "${student_id} # Created: $(date +%Y-%m-%d) Repository: ${repo_name}" >> student-repos/pending-protection.txt
        
        # コミット
        git add student-repos/pending-protection.txt
        git commit -m "Request branch protection setup for ${student_id}

Repository: https://github.com/smkwlab/${repo_name}
Student ID: ${student_id}
Created at: $(date)

This PR was automatically generated by setup.sh"
        ...
        if git push origin "$branch_name" && \
           gh pr create \
               ...
               --base main \
               --head "$branch_name" \
               --title "🔒 Branch protection request: ${student_id}" \
               --assignee toshi0806 \
...
### 自動処理オプション
マージ後に全ての保留中リポジトリを一括処理：
\\\`\\\`\\\`bash
./bulk-setup-protection.sh student-repos/pending-protection.txt
./archive-completed.sh  # 完了分を履歴に移動
\\\`\\\`\\\`

---
*このPRは学生のsetup.sh実行時に自動生成されました*
EOF
)"; then
            echo -e "${GREEN}✅ ブランチ保護設定依頼PR作成完了${NC}"
            echo "   PR URL: https://github.com/smkwlab/thesis-management-tools/pulls"
        else
            echo -e "${YELLOW}⚠️  PR作成に失敗しました（手動で教員に連絡してください）${NC}"
        fi
    else
        echo -e "${YELLOW}⚠️  管理リポジトリのクローンに失敗しました${NC}"
    fi
...
if [ -n "$STUDENT_ID" ]; then
    create_protection_request_pr "$STUDENT_ID" "$REPO_NAME"
else
    echo -e "${YELLOW}⚠️  学籍番号が設定されていないため、自動PR作成をスキップしました${NC}"
