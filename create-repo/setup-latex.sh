#!/bin/bash
# 汎用LaTeXリポジトリ作成スクリプト
# 使用例: /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/smkwlab/thesis-management-tools/main/create-repo/setup-latex.sh)"
...
cleanup() {
    ...
    # Dockerイメージも削除
    if docker images -q latex-setup-alpine >/dev/null 2>&1; then
        echo "🗑️  Dockerイメージをクリーンアップ中..."
        docker rmi latex-setup-alpine >/dev/null 2>&1 || true
    fi
}
...
if [ "${DEBUG:-0}" = "1" ]; then
    # デバッグモードでは詳細出力を表示
    docker build --progress=plain -f Dockerfile-latex -t latex-setup-alpine .
else
    # 通常モードでも進行状況を表示
    if ! docker build --progress=auto -f Dockerfile-latex -t latex-setup-alpine .; then
...
if [ -n "$STUDENT_ID" ]; then
    ...
    if ! docker run --rm -it $DOCKER_ENV_VARS -v "$TOKEN_FILE:/tmp/gh_token:ro" latex-setup-alpine; then
        echo "❌ セットアップスクリプトの実行に失敗しました"
        exit 1
    fi
fi
