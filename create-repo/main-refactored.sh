...
if [ "$INDIVIDUAL_MODE" = false ]; then
    if gh repo create "$REPO_PATH" --private --template="$TEMPLATE_REPOSITORY" --clone; then
    ...
    if gh repo create "$REPO_NAME" --private --template="$TEMPLATE_REPOSITORY" --clone; then
        echo -e "${GREEN}✓ リポジトリ作成完了${NC}"
    else
        echo -e "${RED}エラー: リポジトリの作成に失敗しました${NC}"
        exit 1
