#!/bin/bash
# 論文リポジトリ作成スクリプト (Alpine版)
# 使用例: /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/smkwlab/thesis-management-tools/feat/alpine-based-setup-container/create-repo/setup-alpine.sh)"
...
echo "==============================================="
echo "📚 論文リポジトリ作成ツール (Alpine版)"
echo "🐳 Dockerベース - サイズ最適化"
echo "==============================================="
...
# ブランチ指定がある場合は切り替え
BRANCH="${THESIS_BRANCH:-feat/alpine-based-setup-container}"
...
if [ "${DEBUG:-0}" = "1" ]; then
    ...
    if ! docker build --progress=auto -f Dockerfile.alpine -t thesis-setup-alpine .; then
        echo "❌ Alpine版Dockerイメージのビルドに失敗しました"
        exit 1
    fi
fi

echo "🚀 セットアップ実行中 (Alpine版)..."
...
echo ""
echo "✅ Alpine版でのセットアップが完了しました！"
echo "🐳 従来のUbuntu版と比較してイメージサイズが約1/9に削減されました"